...
!====================================================
!
        subroutine col_MC_masked(itime,opt)
!
        use storage
        use real_kinds
        use timing
...
! only when both bulk and border are upgrated
        if(opt==1) then         
           c01 => a01
           c02 => a02
           c03 => a03
...
#ifdef DEBUG_2
        if(myrank == 0) then
           write(6,*) "DEBUG2: Exiting from sub. col_MC_masked", &
!                       forcex, forcey, forcez, omega            &
                        opt 
        endif
#endif
        end subroutine col_MC_masked
